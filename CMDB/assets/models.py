...
class Asset(models.Model):
    ...
    business_unit = models.ForeignKey('BusinessUnit', null=True, blank=True,
                                      verbose_name='所属业务线', on_delete=models.SET_NULL)
    ...
    expire_day = models.DateTimeField(null=True, blank=True, verbose_name='过保日期')
...
class Server(models.Model):
    ...
    created_by = models.CharField(choices=created_by_choice, max_length=32, default='auto',
                                  verbose_name='添加方式')
    hosted_on = models.ForeignKey('self', related_name='hosted_on_server', blank=True,
                                  null=True, verbose_name='宿主机', on_delete=models.CASCADE)  # 虚拟机专用字段
...
class Software(models.Model):
    ...
    version = models.CharField(max_length=64, unique=True, help_text='例如：RedHat release 7',
...
class Contract(models.Model):
    ...
    end_date = models.DateField('失效日期', blank=True, null=True)
...
class NIC(models.Model):
    ...
    mac = models.CharField('MAC地址', max_length=128)  # 虚拟机有可能会出现同样的mac地址
...
class NewAssetApprovalZone(models.Model):
    ...
    m_time = models.DateTimeField('数据更新时间', auto_now=True)
